<script lang="ts">
	...
	const speedup_rows = derive_speedup_summary(benchmarks_json);
	...
	{:else}
		<section>
			<p>
				...
				After correctness, tsv prioritizes performance and efficiency, and this page measures them.
				For parser correctness see the <TomeLink slug="conformance" /> page.
			</p>
			<p>
				This page compares tsv to <a href="https://prettier.io/">Prettier</a>, which tsv closely
				follows, and to <a href="https://oxc.rs/">Oxc</a> and
				<a href="https://biomejs.dev/">Biome</a>, similar tools with more features and wider
				language support — tsv doesn't support JSX/TSX/SCSS/etc., and it has only one formatting
				style. Also compared: <a href="https://baseballyama.github.io/rsvelte/">rsvelte</a> (Svelte
		...
		<TomeSection>
			...
			<p>
				...
				lands in JS ahead of it; tsv's default AST, which carries the line/column locations Oxc
				omits, lands behind.
			</p>
			<p>
			...
			<ul>
				...
				<li>
					...
					~{format_svelte_vs_biome} faster than Biome, same pairings.
				</li>
				<li>
				...
				<li>
					...
					</a>: on the JSX-free subset of a real TypeScript repo, with every tool launched through
					its npm package's Node bin, tsv formats ~{cli_npm_ts_vs_oxfmt} faster than Oxfmt and
					...
					~{cli_ts_wall_vs_biome} faster. Wall-clock ratios bake in each tool's multi-file
					parallelism — see <a href="#{docs_slugify(CLI_SECTION_TITLE)}">the CLI section</a>.
				</li>
				<li>
					...
					<code>loc</code> for drop-in acorn and Svelte AST compatibility, at ~{parse_ts_loc_cost}
					the parse-and-hand-off time, which puts it behind Oxc's span-only wire (and swc's,
					span-only in its own AST shape).
				</li>
				<li>
					...
					~{parse_svelte_vs_rsvelte} faster than rsvelte's (native-vs-native). CSS runs the other
					way: Svelte's own <code>parseCss</code> is ~{parse_css_compiler_vs_tsv} faster than tsv's
					JSON wire and PostCSS ~{parse_css_postcss_vs_tsv}.
				</li>
				{#if cli_svelte_timed || cli_svelte?.aborted}
					<li>
						...
						{#if cli_svelte_timed}
							There tsv formats ~{cli_svelte_npm_wall} faster through its Node dispatcher, the
							footing rsvelte-fmt's own Node launcher is timed on.
						{:else}
			...
			<p>
				...
				<a href="#{docs_slugify(CROSS_RUNTIME_SECTION_TITLE)}">cross-runtime comparison</a> closes
				the page.
			</p>
		</TomeSection>
		...
		<TomeSection>
			...
			<p>
				...
				and CSS, plus JS through the same TypeScript parser. Each chart's ratios are against its
				highlighted row — hover another row to re-anchor them — and a minus means that many times
				worse.
			</p>
			...
			<aside>
				...
				<ul>
					<li>
						wasm-vs-wasm and native-vs-native (N-API here) are the like-for-like pairings. The
					...
					<li>
						...
						every Svelte file (its platform binary, invoked directly) but isn't timed, since it
						ships no in-process API and a process per file would measure spawn rather than
						formatting.
						{#if cli_svelte_timed}
						...
						{:else if cli_svelte?.aborted}
							...
							<a href="#{docs_slugify(CLI_SECTION_TITLE)}">CLI section</a> is currently aborted
							rather than timed.
						{/if}
					</li>
					<li>
						...
						harness checks that each pin landed. Nothing grades output against an oracle: the bench
						checks that output is non-empty and that tsv's native and wasm builds agree byte for
						byte, so a tool emitting wrong output quickly would still read as fast. tsv's own output
					...
					<li>
						Each format row includes the tool's own parse: source in, formatted text out. Biome's
						only in-process format entry point, <code>formatContent</code>, also opens the file in
						its workspace, pulls syntax diagnostics, and closes it on every call, so its rows carry
						that wrapper too.
					</li>
					<li>
						Oxfmt formats TypeScript, JS, and CSS with its own native engine; for Svelte it
						delegates to a Prettier it bundles, with a bundled copy of
						<a href="https://github.com/sveltejs/prettier-plugin-svelte">prettier-plugin-svelte</a>
						— only the embedded <code>&lt;script&gt;</code> goes through its native engine; the
					...
					<li>
						...
						only the script. Embedded script and style are formatted too, so the work is comparable.
						Its row also pays a cold first sweep each time the harness rebuilds its wasm instance, a
						few percent. There's no native Biome entry: its in-process API,
						<code>@biomejs/js-api</code>, runs only on its wasm builds, and the native engine ships
						only as the <code>biome</code> CLI, a separate process rather than a library.
					</li>
					<li>
						The dprint entry is
						...
						plugin. It rejects CSS and Svelte, and no dprint markup plugin is wired in, so it has no
						Svelte row and its CSS slot goes to
		...
		<TomeSection>
			...
			<p>
				...
				they show raw engine speed, and their gap to the JSON rows is what serializing and handing
			...
			<p>
				...
				binary buffer its JS side decodes into objects lazily — so the bench forces the whole tree
				and times that; the deliverable is the same fully materialized tree, so the entries stay
				comparable.
			</p>
			...
			<aside>
				...
				<ul>
					...
					<li>
						...
						<code>reconstruct_locations</code> is faster than tsv's default <code>loc</code>-bearing
						wire — see the
					...
					<li>
						...
						expressions and keeps the top-level offsets — a different trade than tsv's span-only
						wire.
					...
					<li>
						...
						the parser behind Prettier's CSS printer, so it's the parse-side counterpart of the
						Prettier format entry. It isn't payload-matched to tsv: it keeps selectors and values as
						strings where <code>parseCss</code> and tsv build ASTs, a much lighter tree.
					</li>
				</ul>
			</aside>
		</TomeSection>
		...
		<TomeSection>
			...
			<p>
				...
				<code>gz</code> annotation beside each is the gzipped size, the better estimate of a
				download.
			</p>
			...
			<aside>
				...
				<ul>
					<li>
						...
						loads. The full entry is barely larger than the formatter because the formatter already
						contains the Svelte parser. Minified JS compresses much better than wasm, so these read
						smaller by <code>gz</code> than by raw bytes.
					</li>
					<li>
					...
					<li>Biome bundles a parser, formatter, and linter for many languages.</li>
					<li>
						...
						equivalent to tsv's single parse+format build. That sum is a little unfair to oxc: oxfmt
						statically links its own copy of the oxc parser, so the two builds count the parser
						twice. The other way, the <code>oxfmt</code> addon is more than a formatter for tsv's
						three languages — it also formats JSON and YAML, sorts imports, and carries a language
						server and the CLI itself.
					</li>
					<li>
						...
						the platform package that ships it also carries the <code>tsv</code> CLI binary, nearly
						as large again.
					</li>
					<li>
					...
					<li>
						<code>{RSVELTE_LABEL}</code> is a scope mismatch of its own kind — a standalone
						executable carrying a CLI and oxc's JS/TS, CSS, and JSON formatters beside its Svelte
						engine, where the tsv entries are bare libraries. <code>{RSVELTE_INSTALL_LABEL}</code>
						is what you install to format a project, and the fairer comparison to tsv's single
						build: over a directory it needs oxfmt, an optional peer dependency it hands every file
						type it doesn't format itself.
					</li>
					<li>
		...
		<TomeSection>
			...
			<p>
				...
				intersection; where a group runs short of the corpus total, a note under its chart gives the
				files and the share of the group's bytes left out, and which rows failed them.
			</p>
			<p>
			...
			<p>
				Each row gets a time budget rather than a sweep count: at least
				{format_count_maybe(sweeps.floor)} sweeps{canonical_floor_note}, otherwise as many as fit a
				few seconds, so the multi-second rows stop near that floor. After outlier cleaning, rows
				keep from {format_count_maybe(sweeps.sample_size_min)} to
				{format_count_maybe(sweeps.sample_size_max)} timings. A low cv over a handful of sweeps is
				thinner evidence than the same cv over hundreds, so the instability check proves less for
				the slow rows — Prettier among them, the denominator of every ratio in the summary table.
			</p>
			<p>
				...
				Against every alternative that bias counts for tsv; against the canonical row, which runs
				first, it counts against tsv — in the summary table, and the parse comparisons against
				svelte/compiler, acorn-typescript, and <code>parseCss</code>.
			</p>
			<p>
			...
			<p>
				...
				across {benchmarks_cli.machine.threads} threads, which its multi-file wall-clock scales
				with.
		...
		<TomeSection>
			...
			<p>
				...
				ecosystem{corpus_counts.harvested_css && corpus.css
					...
					: ''}. Test files count as real code and stay in; fixture files (formatter test suites,
				and fixture subtrees inside the measured repos) are excluded — deliberately tricky edge
				cases measure conformance, not typical throughput, and the <TomeLink slug="conformance" />
				page covers them.
			</p>
			<p>
				Two caveats. The corpus is dominated by the author's own code plus Svelte's, the same code
				tsv is developed and tested against and mostly tsv-formatted already, so every ratio here is
				"on this corpus", not a universal figure; the CLI section's Svelte corpus adds those
				third-party libraries, and shares only its kit and svelte.dev trees with this one. And CSS
				is the weakest sample: just {format_count_maybe(corpus_counts.standalone_css)} standalone
				files beside those extractions.
			</p>
		...
		<TomeSection>
			...
			<p>
				...
				implementation, or the binding boundary — not a difference in tsv's algorithms. A delta the
				report finds inside the two measurements' combined noise is marked <code>≈</code> in the
				tables and reads as parity, not an effect. It can't see variance between whole runs: Bun's
				allocation-heavy JS rows (Prettier, PostCSS) have sat at two levels ~10–14% apart.
			</p>
			<aside>
				<p>
					The <code>tsv-internal</code> rows cross the native binding boundary but materialize
					nothing on the JS side, so a delta there is the boundary plus each engine's hand-off of
					the source string into it, nothing more; the JSON-materializing parse rows add each JS
					engine's <code>JSON.parse</code> cost on top. Node is the headline as the default N-API
					path, not a speed pick.
				</p>
			</aside>
