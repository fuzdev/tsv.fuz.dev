...
const parse_preflight = (block: string): Array<FormatterPreflight> =>
	[
		...slice_section(block, 'Preflight (per-formatter parse check):', 'Benchmark 1:').matchAll(
			PREFLIGHT_RE,
		),
	].map((m) => ({
		name: m[1]!,
...
const parse_scenario = (name: string, block: string): FormatterScenario => {
	...
	return {
		...
		preflight: parse_preflight(block),
		timings: parse_timings(block),
 ...
 * @returns the parsed benchmarks, or `undefined` if the results markers are
 * missing or no tsv scenario was found
 */
export const parse_formatter_benchmarks = (readme: string): FormatterBenchmarks | undefined => {
	...
	const scenarios = headings
		.map(({0: heading, 1: name, index}, i) =>
			parse_scenario(name!.trim(), results.slice(index + heading.length, headings[i + 1]?.index)),
		)
		.filter((scenario) => scenario.timings.some((timing) => timing.name === 'tsv'));
	...
	return {machine: MACHINE_RE.exec(readme)?.[1]?.trim() ?? '', versions, scenarios};
};
